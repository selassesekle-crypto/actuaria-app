...
class AgentA6Comparaison:
    ...
    def run(
        ...
        # ⚠️⚠️ IL Y A EU UN `observation_qualite` ICI, RETIRÉ LE 02/09/2026. Il
        # transitait « ce que la couche AURAIT fait » vers les trois rapports,
        # le temps d'une étape de MESURE (1-B-④). L'étape ⑤ a branché la couche
        # sur le chemin agent : elle s'APPLIQUE, `rapport_qualite` ci-dessus dit
        # ce qu'elle a fait, et le canal n'avait plus rien à porter.
        # *Un canal que plus rien ne remplit se lit `None` sans jamais échouer :
        # c'est la panne la plus silencieuse qui soit.* `QNE-9` tient le retrait.

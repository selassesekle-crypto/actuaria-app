...
class T3_RiskAdjustment_VaR(unittest.TestCase):
    ...
    def test_ra_positif_et_raisonnable(self):
        r = run_ok()
        ra  = r['provisions']['risk_adjustment']
        be  = A7['best_estimate']['best_estimate']
        self.assertGreater(ra, 0)
        # RA doit être entre 2% et 15% du BE (bornes raisonnables)
        self.assertGreater(ra, be * 0.02)
        self.assertLess(ra, be * 0.15)
        # Quantile déclaré
        self.assertEqual(r['provisions']['quantile_ra'], 0.75)
        print(f"    ✅ T3 RA : {ra:,.0f}€  ({ra/be*100:.1f}% du BE) | P75 déclaré")

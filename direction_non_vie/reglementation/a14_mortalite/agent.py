...
def _generer_th0002() -> np.ndarray:
    """Table TH0002 officielle W-H — Hommes rentes viageres (arrete 27/07/2006)."""
    q = np.zeros(116)
    for x in range(111):
        q[x] = _QX_TH0002_DICT.get(x, 1.0)
    q[115] = 1.0
    return q

def _generer_tf0002() -> np.ndarray:
    """Table TF0002 officielle W-H — Femmes rentes viageres (arrete 27/07/2006)."""
    q = np.zeros(116)
    for x in range(111):
        q[x] = _QX_TF0002_DICT.get(x, 1.0)
    q[115] = 1.0
    return q

def _generer_tghf05(sexe: str = 'H') -> np.ndarray:
    """Table generationnelle plate (INSEE 2014, annee ref 2025)."""
    q = np.zeros(116)
    for x in range(116):
        q[x] = insee_qx_prospectif(x, 2025, sexe)
    q[115] = 1.0
    return q

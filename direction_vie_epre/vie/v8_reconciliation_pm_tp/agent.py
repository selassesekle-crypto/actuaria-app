...
#              Impact ≈ PM × (frais_social - frais_s2) × ä_residuelle
    ...
    def _generer_excel(self, result: dict) -> bytes:
        """Rapport Excel individuel V8 — 4 onglets auditables."""
        from datetime import datetime
        ...
        ajouter_onglet_hypotheses(wb, "V8", aid, dte, hyps,
        ...
        ajouter_onglet_resultats(wb, "V8", aid, dte,
        ...
        for vk in [k for k in result if "validat" in k.lower()]:
            ...
            if isinstance(val, dict) and "statut_global" in val:
                ...
                if cles:
                    ajouter_onglet_validation(wb, "V8", aid, dte, val,
                                              cles_controles=cles[:5])
                break
        ajouter_onglet_audit_trail(wb, "V8", aid, dte, result)

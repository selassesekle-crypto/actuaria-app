...
def generer_contenu_notebook(agent: dict) -> dict:
    ...
    cellule_config = {
        ...
        "source": [
            ...
            "BRANCHE = 'non_vie'\n",
            "\n",
            "# Sous-branche (LoB) — DÉCLARÉE depuis la Phase 1 (A1 ne la devine plus)\n",
            "# Options Non-Vie : 'auto', 'mrh', 'rcpro', 'decennale', ...\n",
            "SOUS_BRANCHE = 'auto'\n",
            "\n",
            "# Fichier à charger\n",
...
def generer_cellule_execution(agent: dict) -> dict:
    ...
    # ── A2 : PREPROCESSING ────────────────────────────────────────────────────
        ...
        source = [
            ...
            "    verbose    = False\n",
            ")\n",
            "result_a1 = agent_a1.run(branche=BRANCHE, sous_branche=SOUS_BRANCHE, fichier=FICHIER)\n",
            "print(f\"A1 : {result_a1['statut_rag']} | {len(result_a1['dataframe']):,} lignes\")\n",
            "\n",
            "# Étape 2 : Preprocessing avec A2 — piloté par le PLAN signé (Phase 2)\n",
            "from core.plan_tarifaire import PlanTarifaire\n",
            "_plan = PlanTarifaire.depuis_yaml(f'{BASE_PATH}/plans/{SOUS_BRANCHE}.yaml')  # adapter le chemin des plans si besoin\n",
            "agent_a2 = AgentA2Preprocessing(\n",
            "    models_path = MODELS_PATH,\n",
            "    audit_path  = AUDIT_PATH,\n",
            "    verbose     = True\n",
            ")\n",
            "result = agent_a2.run(result_a1, plan=_plan)\n",
            "result_a2 = result\n",
